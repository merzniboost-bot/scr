...
class TestMarkdownToHtml:
    """Тесты для функции markdown_to_html()"""
    
    def test_basic_formatting(self, app):
        """Базовое форматирование: жирный, курсив"""
        from utils import markdown_to_html
        
        with app.app_context():
            # Жирный текст
            result = markdown_to_html('**жирный текст**')
    ...
    def test_underline(self, app):
        """Подчёркивание через __text__"""
        from utils import markdown_to_html
        
        with app.app_context():
            result = markdown_to_html('__подчёркнутый__')
            assert '<u>' in result
            assert 'подчёркнутый' in result
...
```"""
    ...
    def test_inline_code(self, app):
        """Инлайн код"""
        from utils import markdown_to_html
        
        with app.app_context():
            result = markdown_to_html('Используйте `print()` для вывода')
            
            assert '<code>' in result
            assert 'print()' in result
    
    def test_colored_text(self, app):
        """Цветной текст с сохранением style"""
        from utils import markdown_to_html
        
        with app.app_context():
            result = markdown_to_html('<span style="color: #FF0000">красный</span>')
            
            assert '<span' in result
            assert 'color' in result
            assert 'красный' in result
    
    def test_code_block_preserves_content(self, app):
        """Блок кода сохраняет содержимое без интерпретации"""
        from utils import markdown_to_html
        
        with app.app_context():
            md_text = """```
**не жирный** и <script>alert(1)</script>
```"""
            result = markdown_to_html(md_text)
            
            # Скрипт должен быть очищен
            assert '<script>' not in result
            # Markdown внутри кода НЕ должен рендериться
            # (** должны остаться как текст, не как <strong>)
