...
def markdown_to_html(content, safe_mode=True):
    ...
    - Нумерованные (1. 2. 3.) и маркированные списки
    - Блоки кода ```code``` и инлайн `code`
    - Цветной текст <span style="color: #FF0000">...</span>
    - Подчёркивание через __text__ (конвертируется в <u>)
    """
    if not content:
        return ""
    ...
    try:
        ...
        # Конвертируем __text__ в <u>text</u> ДО парсинга Markdown
        # (т.к. Markdown превращает __ в <strong>)
        # Паттерн: __текст__ но НЕ внутри блоков кода
        def convert_underline(text):
            ...
            # Сохраняем fenced code blocks и inline code
            text = re.sub(r'```[\s\S]*?```', save_code, text)
            text = re.sub(r'`[^`]+`', save_code, text)
            
            # Конвертируем __text__ в <u>text</u>
        ...
        # 'extra' включает: fenced_code, tables, attr_list, def_list, footnotes
        # Добавляем 'tilde' для ~~зачёркивания~~ через модуль pymdownx если доступен
        extensions = ['extra', 'fenced_code', 'tables', 'nl2br', 'codehilite']
        
        # Пробуем добавить поддержку strikethrough
        try:
            import pymdownx.tilde
            extensions.append('pymdownx.tilde')
        except ImportError:
            # Если pymdownx недоступен, используем fallback через regex
        ...
        if safe_mode:
            ...
            allowed_tags = base_tags + [
                ...
                'u', 'ins', 'del', 'mark', 'sub', 'sup'  # Дополнительные теги форматирования
            ]
            ...
            # Используем CSSSanitizer для сохранения стилей (color, background и др.)
            css_sanitizer = None
            ...
            if css_sanitizer:
                ...
                # Fallback для старых версий bleach
