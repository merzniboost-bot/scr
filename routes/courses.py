...
from models import db, Course, Lesson, Test, Question, User, UserProgress, Favorite
from decorators import login_required, role_required, course_owner_required
from utils import save_file, delete_file, allowed_file, get_course_progress, is_course_favorite, get_lesson_progress_map
...
def course_students(course_id):
    ...
    try:
        ...
        # Получаем уникальных студентов с прогрессом
        students_data = db.session.query(
            User,
            db.func.count(UserProgress.id).label('total_progress'),
            db.func.sum(db.case((UserProgress.completed == True, 1), else_=0)).label('completed')
        ).join(UserProgress, User.id == UserProgress.user_id) \
            .filter(UserProgress.course_id == course_id) \
            .group_by(User.id) \
            .order_by(User.fullname).all()

        # Рассчитываем процент завершения для каждого студента
        total_lessons = len(course.lessons)

        students = []
        for user, total_prog, completed in students_data:
